...
class NodeTest : TestBase() {
	...
	fun equals_different_type() {
		...
		assertFalse(xml.equals(other))
	}

	@Test
	fun equals_different_name() {
		val xml1 = xml("test1")
		val xml2 = xml("test2")
	...
	@Test
	fun addElementsAfter_not_found() {
	...
	fun addElementsAfter() {
		...
		validate(
			...
			"NodeTest/addElementsAfter.xml"
		)
	}

	@Test
	fun addElementsBefore_not_found() {
		assertFailsWith<IllegalArgumentException> {
			val xml = xml("root")
			val text = TextElement("test")
			xml.addElements(text)
	...
	fun addElementsBefore() {
		...
		validate(
			...
			"NodeTest/addElementsBefore.xml"
		)
	}
}
