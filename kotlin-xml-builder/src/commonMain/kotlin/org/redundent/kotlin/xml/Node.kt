...
open class Node(val nodeName: String) : Element {
	...
	protected var childOrder: Array<String> = emptyArray()
	...
	var xmlns: String?
		...
		set(value) {
			if (value != null) {
				...
				val iterator = _namespaces.iterator()
				while (iterator.hasNext()) {
					val namespace = iterator.next()
	...
	fun attributes(vararg attrs: Attribute) {
		for (attr in attrs) {
			val namespace = attr.namespace
			if (namespace != null) {
				addNamespace(namespace)
			}
			attribute(buildName(attr.name, namespace), attr.value)
		}
	}
